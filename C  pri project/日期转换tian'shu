...
int main(void)
{	
	puts("这个程序返回你输入日期的天数,特殊日期可以触发彩蛋！");
	start();
	return 0;
}
void killline(void)
...
void start(void)
{
	...
	{
		puts("请输入指定日期的年份：");
		while (scanf_s("%d", &yeartemp) != 1 || yeartemp > 2018 || yeartemp <= 0)
		{
			puts("输入错误，重新输入");
			killline();
		}
		...
		totaldays = mymonth[fmon(montemp) - 1].total + daytemp;
		printf("%d年到%d年%d月%d日总共%d天\n", yeartemp, yeartemp, fmon(montemp), daytemp, totaldays);
		if (yeartemp == 1992 && daytemp == 10 && fmon(montemp) == 6)
		{
			printf("\n\n\n");
			printf("\t\t");
			star(17);
